...
Access-constraint proxy:

~~~text
tube length
~~~

Bypass-response proxy:

~~~text
(robbing - thieving) / (robbing + thieving)
~~~
...
Each network contributes one rank correlation.

To prevent the much larger Aubert/EPHI dataset from overwhelming the Sakhalkar network solely by sample size, networks receive equal weight.
...
Also report the arithmetic mean correlation as a descriptive sensitivity.
...
~~~text
shuffle bypass-response values across plant species
~~~

Aubert/EPHI:

~~~text
shuffle robbery-rate values within site only
~~~
...
- identical raw effect scales;
- one common sampling variance;
- one common visitor guild;
- that 1,378 Aubert pair-sites equal 1,378 independent networks.

The inferential claim is therefore:
...
`scripts/analyze_joint_access_routing.py`

Tests:

`tests/test_joint_access_routing.py`

Workflow:

`.github/workflows/analyze-joint-access-routing.yml`
