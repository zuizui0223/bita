...
def _candidate(candidate_id: str, *, priority: bool) -> Candidate:
    return Candidate(
...
def test_collector_records_rank_binned_metadata_yield() -> None:
    ...
    collector.observe(_query(), 0, [_candidate("p", priority=True), _candidate("u", priority=False)], 2)
    collector.observe(_query(), 2, [_candidate("p2", priority=True)], 1)
