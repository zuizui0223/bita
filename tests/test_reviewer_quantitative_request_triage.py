...
def test_bita_triage_prioritizes_identification_gain() -> None:
    text = TRIAGE.read_text(encoding="utf-8")
    for token in (
        "Kessler reconstruction sensitivity",
        "partial-identification bounds",
        "crossed-intervention power",
        "pool 56/25/17 into prevalence",
        "residual-by-subtraction mechanism",
    ):
        assert token in text


def test_bita_triage_preserves_identification_ceiling() -> None:
