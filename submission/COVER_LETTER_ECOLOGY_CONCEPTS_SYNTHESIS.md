...
Please consider our manuscript, **“When does a trait trade-off resolve by differentiation rather than compromise? Linking trait architecture to mechanism identification,”** for publication as a **Concepts & Synthesis** article in *Ecology*.

The manuscript addresses a general problem in ecological trait evolution. When one trait contributes to functions with different preferred phenotypic states, selection can be resolved within the existing architecture by compromise, or the functions can become more separable across multiple trait axes. The first solution is the balance problem developed in the companion SCH programme. Here we ask when the second solution—trait differentiation—actually pays, and what can be inferred mechanistically once several trait axes exist.

The first contribution is an explicit architecture comparison. If a differentiated architecture contains the shared phenotype on its diagonal, its optimized pre-cost fitness cannot be lower than the best shared compromise. Writing `R` for the loss recoverable from the shared compromise and `K` for the extra fixed architecture cost gives the general decision `Delta_arch = R-K`. A non-negative residual-coupling penalty can only reduce `R` as coupling strengthens. In a quadratic baseline, the result becomes especially interpretable: the recoverable loss is `R=sL_S*`, where `L_S*` is the shared-axis conflict load and `s` is the fraction of function-specific trait separation that survives residual coupling. Thus incomplete differentiation, rather than perfect modular independence, is part of the central model.

We distinguish these structural results from the response-shape-dependent claims. A registered 300-condition convex power-loss analysis tests strict positive recovery and the effect of increasing distance between function-specific optima. Strict recovery occurs in all 300 nonzero-conflict evaluations, and recoverable loss increases with optimum separation in all 60 declared comparison series. The 60/60 coupling result is treated as an implementation check of the structural monotonicity proposition rather than as evidence for universality. We explicitly do not claim a general evolutionary-dynamics theorem or historical reconstruction from these optimized-state comparisons.
...
**shared-axis balance → partial trait differentiation → mechanism identification.**
