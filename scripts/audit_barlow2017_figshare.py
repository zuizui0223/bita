...
metadata only. Observation-level rows and non-header numeric cell values are never
written to the artifact. For Excel sheets with title rows, a header row is selected
using string cells only from the first 15 rows.
"""
...
USER_AGENT = "bita-barlow2017-figshare-audit/1.2"
...
def _audit_text(file: dict[str, object]) -> dict[str, object]:
    ...
    result: dict[str, object] = {
        "file_id": file.get("id"), "file_name": name, "size_bytes": file.get("size"),
        "suffix": _suffix(name), "download_url_present": bool(url),
    }
    ...
    result.update({
        ...
        "n_rows": sum(1 for _ in reader), "headers": headers,
...
def _string_cell_text(cell: ET.Element, shared: list[str]) -> str:
    """Return string/inline-string content only; ignore numeric/formula values."""
...
def _header_candidate(sheet_data: ET.Element | None, shared: list[str]) -> tuple[int | None, list[str]]:
    if sheet_data is None:
        return None, []
    best: tuple[tuple[int, int, int], int, list[str]] | None = None
    for row in list(sheet_data.findall(f"{{{NS_MAIN}}}row"))[:15]:
        row_number = int(row.attrib.get("r", "0") or 0)
        strings = [_string_cell_text(cell, shared) for cell in row.findall(f"{{{NS_MAIN}}}c")]
        strings = [value for value in strings if value]
        if not strings:
            continue
        token_hits = sum(any(token in value.lower() for token in TOKENS) for value in strings)
        # Prefer multi-column descriptive rows over single-cell title rows.
        score = (int(len(strings) >= 2), len(strings), token_hits)
        if best is None or score > best[0]:
            best = (score, row_number, strings)
...
def _audit_xlsx(file: dict[str, object]) -> dict[str, object]:
    ...
    for sheet_name, path in _sheet_targets(book):
        ...
        header_row, headers = _header_candidate(sheet_data, shared)
        sheets.append({
    ...
    result["schema_guardrail"] = (
        "Header selection scanned string cells only in the first 15 rows; numeric/formula observation values "
        "were ignored and no non-header cell values were emitted."
    )
    return result


def _audit_file(file: dict[str, object]) -> dict[str, object]:
    return _audit_xlsx(file) if _suffix(str(file.get("name") or "")) == ".xlsx" else _audit_text(file)
...
def run(output_path: str | Path) -> dict[str, object]:
    ...
    if not isinstance(files, list):
        files = []
    report = {
        "source_doi": SOURCE_DOI, "declared_data_doi": DATA_DOI,
        "figshare_article_id": ARTICLE_ID, "title": article.get("title") if isinstance(article, dict) else None,
        ...
        "file_count": len(files), "files": [_audit_file(item) for item in files if isinstance(item, dict)],
        "guardrails": [
            "No observation-level rows or non-header numeric/formula values are written to the report.",
