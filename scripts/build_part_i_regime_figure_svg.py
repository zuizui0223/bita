"""Build manuscript Figure 2 from reproduced Part I robustness CSV outputs.

The figure is intentionally dependency-free SVG so CI can regenerate it without
plotting-library drift. It contains three panels:
...
C. Functional-form sensitivity in complementary fraction.
...
def build_svg(rows: list[dict[str, str]]) -> str:
    if not rows:
        raise ValueError("robustness cases are empty")
    ...
    # Panel B: service x damage heatmap
    ...
    # Panel C: functional forms
    form = grouped_fraction(rows, ("form_id",))
    x2, y2, pw2, ph2 = 830, 95, 430, 520
    parts += [_text(x2, y2 - 22, "C  Functional-form sensitivity", size=18, weight="bold"),
    ...
    parts.append(_text(55, 690, "Bars and cells show theoretical sign frequencies over the predeclared grid; they are not empirical probabilities.", size=14))
    parts.append("</svg>")
    return "\n".join(parts) + "\n"
