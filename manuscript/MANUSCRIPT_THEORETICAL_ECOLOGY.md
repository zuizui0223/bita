...
This paper makes five linked contributions.
...
4. **Finite sensitivity analysis.** We evaluate the implemented corollary across endpoint-normalized response shapes and contrasting biological scenarios, using the finite design only as a robustness/sensitivity diagnostic.

5. **Mechanism-pattern empirical synthesis.** Rather than treating one marginal route as validation of the theory, we audit the recurrence, same-system co-occurrence, conditionality, direct interaction evidence, quantitative mechanism modules, and identification gaps implied by the theoretical architecture. The empirical endpoint is a map of what is recurrent, what is context dependent, and what remains empirically unidentified.

### 1.6 Paper organization

Section 2 defines the focal traits and outcome scale, develops the signed channel decomposition and orientation criterion, establishes mechanism non-identifiability, derives environmental comparative statics, specifies the finite sensitivity design, and defines the mechanism-pattern empirical synthesis. Section 3 presents the theoretical results followed by the source-adjudicated empirical architecture and two quantitative synthesis modules. Section 4 integrates the theoretical and empirical results, clarifies the distinction from correlational selection, and identifies the experiments needed to estimate the complete mixed partial. Section 5 summarizes the conclusions.
...
### 4.1 A conditional sign boundary is biologically necessary, not merely mathematically possible

The theory shows that local complementarity requires antagonist relief to exceed pollinator interference plus direct joint-cost curvature. The empirical synthesis shows why each side of that inequality must remain open. Source-adjudicated systems contain defence states that reduce antagonism with little detected pollinator cost, states in which pollinator interference appears only above a trait-expression threshold, and states in which reward context, exposure duration, response definition, or consumer identity changes the observed outcome. Attraction can likewise track mutualists, antagonists, or both.
...
### 4.2 Constituent-path evidence is not validation of the mixed partial
...
### 4.3 Direct evidence scarcity is itself informative
...
### 4.4 Relation to correlational selection and fitness-landscape inference
...
### 4.5 Environmental variables should be treated as joint ecological states
