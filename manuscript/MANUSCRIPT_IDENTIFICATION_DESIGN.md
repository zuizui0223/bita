...
The resulting framework closes the established sequence **interaction detection → partial identification → mechanism identification**, while inserting an explicit outcome-level escape-sign decision at the interaction stage when design-based uncertainty excludes zero. It moves floral attraction-defence research from detecting non-additivity and cataloguing recurrent pathways to stating exactly what current evidence constrains, what remains unidentified, and which additional observation would resolve it.
